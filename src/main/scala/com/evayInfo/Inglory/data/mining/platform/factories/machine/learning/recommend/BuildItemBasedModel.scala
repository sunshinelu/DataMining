...
class BuildItemBasedModel {
  ...
  def ItemBased(train_table:String,user_col:String,item_col:String,rating_col:String,
    ...
    val SparkConf = new SparkConf().setAppName(s"BuildItemBasedModel:ItemBased").setMaster(spark_master).set("spark.executor.memory", spark_executor_memory)
  ...
  def ItemBased_test(train_table:String,user_col:String,item_col:String,rating_col:String,
    ...
    val SparkConf = new SparkConf().setAppName(s"BuildItemBasedModel:ItemBased_test").setMaster(spark_master).set("spark.executor.memory", spark_executor_memory)
    val spark = SparkSession.builder().config(SparkConf).getOrCreate()
    val sc = spark.sparkContext
    import spark.implicits._
