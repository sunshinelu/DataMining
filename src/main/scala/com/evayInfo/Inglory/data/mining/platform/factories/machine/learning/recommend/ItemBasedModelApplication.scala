...
class ItemBasedModelApplication {
  ...
  def RatingPrediction(test_table: String, model_name: String,
                       ...
                       opt_table: String)={
    val SparkConf = new SparkConf().setAppName(s"ItemBasedModelApplication:RatingPrediction").setMaster(spark_master).set("spark.executor.memory", spark_executor_memory)
  ...
  def TopNProductsForUsers(test_table: String, model_name: String,
    ...
    val SparkConf = new SparkConf().setAppName(s"ItemBasedModelApplication:TopNProductsForUsers").setMaster(spark_master).set("spark.executor.memory", spark_executor_memory)
    val spark = SparkSession.builder().config(SparkConf).getOrCreate()
    val sc = spark.sparkContext
    import spark.implicits._
  ...
  def TopNUsersForProducts(test_table: String, model_name: String,
                           user_col: String, item_col: String, rating_col: String,
                           topN:Int,opt_table: String)={
    val SparkConf = new SparkConf().setAppName(s"ItemBasedModelApplication:TopNUsersForProducts").setMaster(spark_master).set("spark.executor.memory", spark_executor_memory)
    val spark = SparkSession.builder().config(SparkConf).getOrCreate()
    val sc = spark.sparkContext
    import spark.implicits._
