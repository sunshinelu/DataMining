package com.evayInfo.Inglory.data.mining.platform.factories.machine.learning.classification

/**
 * Created by sunlu on 18/11/23.
 */
class ClassificationAlgorithm {

  def classification(algorithm_option:String,algorithm_mode:String,
                     trainTable:String,col_names:String,testTable:String,optTable:String,
                     filePth:String,modelName:String,
                     param0:String,param1:String,param2:String,param3:String
                      ): Unit ={

  }

}
